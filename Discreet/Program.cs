	...
	class Program
	{
		...
		*/
		...
		public static object GetBalance(StealthAddress address)
        {
            ...
            {
				if (addr.Address == address.ToString())
                {
					if (addr.Encrypted)
                    {
						return new RPCError(-1, "could not get balance because wallet is encrypted");
                    }
		...
		[RPCEndpoint(endpoint_name: "lockWallet")]
		public static bool LockWallet()
        {
			visor.wallet.Encrypt();

			return true;
        }

		[RPCEndpoint(endpoint_name: "unlockWallet")]
		public static object UnlockWallet(string passphrase)
        {
			try
            {
				visor.wallet.Decrypt(passphrase);

				return true;
			}
			catch (Exception e)
            {
				return new RPCError(-1, "wallet could not be unlocked: " + e.Message);
            }
		}

		[RPCEndpoint(endpoint_name: "sendTransaction")]
		public static object SendTransaction(StealthAddress to, ulong amount)
        {
			try
            {
				var tx = visor.wallet.Addresses[0].CreateTransaction(to, amount);

				_ = visor.SendTransaction(tx.ToFull());

				var err = tx.ToFull().Verify();
            ...
            {
				return new RPCError(-1, "could not create transaction: " + e.Message);
			}
		}
		...
		[RPCEndpoint(endpoint_name: "sendMessage")]
		...
		public static async Task Main(string[] args)
        {
			...
			if (!File.Exists(Path.Combine(visorConfig.WalletPath, $"{label}.dis")))
            {
				string _yN;
				bool _yes;
				...
				{
					Mnemonic _mnemonic = new Mnemonic(256, Mnemonic.Language.English);
				...
				if (mnemonic != null)
				{
					w = new Wallet(label, passphrase, mnemonic, bip39: (uint)mnemonic.Split(' ').Length, encrypted: true, deterministic: true, numStealthAddresses: (uint)1, numTransparentAddresses: (uint)0);
				}
				else
				{
					w = new Wallet(label, passphrase, new Mnemonic(entropy).GetMnemonic(), bip39: (uint)(new Mnemonic(entropy).Words.Length), encrypted: true, deterministic: true, numStealthAddresses: (uint)1, numTransparentAddresses: (uint)0);
				}
